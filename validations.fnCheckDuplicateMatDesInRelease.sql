...
-- Description:	Returns the list of Material Description  which are not unique for a particular release
--				More Information here: 
-- =============================================
CREATE FUNCTION [validations].[fnCheckDuplicateMatDesInRelease] 
(
...
BEGIN
	...
	[Validation Name] = 'Material Description should not be duplicated in a single release',	
	[Flagged Column Name] = 'Material Description',
	[Flagged Column Value] = s.[Material Description],							 
	[Remarks] = 'Duplicate Material Description found',
	[SKU State] = s.[State],	
...
GROUP BY e.[Cayman Release],s.[Material Description],s.ConsumptionSkuID,  e.ID, m.MeterID, s.[State], e.[SAP Rate Start Date], m.[Meter Status]
